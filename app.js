...
// 公開CSVを使用する場合（まだなら空文字 "" のままでOK）
const KPI_CSV_PUBLISHED   = ""; // 例: "https://docs.google.com/spreadsheets/d/.../pub?output=csv"
const TREND_CSV_PUBLISHED = ""; // 例: "https://docs.google.com/spreadsheets/d/.../pub?gid=709246046&single=true&output=csv"
...
async function fetchCsvWithFallback(gid, publishedUrl) {
  const urls = [
    publishedUrl ? `${publishedUrl}&cb=${Date.now()}` : null,
    `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/export?format=csv&gid=${gid}&cb=${Date.now()}`,
    `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?gid=${gid}&tqx=out:csv&cb=${Date.now()}`
  ].filter(Boolean);
...
function norm(a) {
  return a.map(v => (Number(v) <= 1 ? Number(v) * 100 : Number(v)));
}
...
async function loadKpi() {
  try {
    ...
    toast("KPIの取得に失敗しました（権限またはネットワーク）");
  }
}
...
async function loadTrend() {
  try {
    ...
    toast("Trendの取得に失敗しました（権限またはネットワーク）");
  }
}
...
function renderPies() {
  ...
  const mkCfg = (val, color, label) => ({
    ...
    options: {
      responsive: true,
      maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: (ctx) => `${label}：${ctx.label} ${ctx.parsed}%` } } }
    }
  });
...
function setupUI() {
  ...
  yearSelect.addEventListener("change", () => {
    if (document.getElementById("btnPie").classList.contains("active")) renderPies();
  });
}
